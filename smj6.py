...
def play(result):
	...
	try:
		call(['/opt/local/bin/mplayer', result[3]])
...
def getMediaFilesFromLocation(location):
	...
	results = Popen('find ' + location + ' \\( -iname *.m4a -o -iname *.mp4 -o -iname *.mp3 -o -iname *.ogg -o -iname *.oga -o -iname *.flac \\)', shell = True, stdout = PIPE).communicate()[0].splitlines()
...
def getNewerMediaFilesFromLocation(location, referenceFile):
	...
	location = getRealPath(location)
	results = Popen('find ' + location + ' -newer ' + referenceFile + ' \\( -iname *.m4a -o -iname *.mp4 -o -iname *.mp3 -o -iname *.ogg -o -iname *.oga -o -iname *.flac \\)', shell = True, stdout = PIPE).communicate()[0].splitlines()
...
def populateDB(dbFile, mediaLocation, mode = 'initial'):
	...
	try:
		c.executemany("insert into media (track, artist, album, file) values (:track, :artist, :album, :file)", mediaIndexer(songFiles, mediaLocation))
