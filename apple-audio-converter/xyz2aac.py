...
try:
    ...
    from subprocess import call # Popen, PIPE removed
...
# Provide a default list of common AAC formats as afconvert might not be available
# or accessible during import time. This makes the script more portable and testable.
...
parser.add_argument(
    "-q", "--quality", type=int, default=75, help="VBR quality, in percent [75]"
)

parser.add_argument(
    "-b", "--bitrate", type=int, default=128, help="average bitrate, in KB/s [128]"
)
...
def get_audio_files(location):
    for path, dirs, files in walk(location):
        for f in files:
            if (
                f.endswith(".m4a") or f.endswith(".mp3") or f.endswith(".flac")
...
def convert_flac_to_wav(flac_file, wav_file):
    logger.debug(
        'Converting FLAC file to intermediate WAV file: "%s".'
...
def convert_wav_to_aac(wav_file, m4a_file, lossless=args.lossless):
    ...
    if args.lossless:
        ...
        logger.debug(
            'Converting "%s" to a "%s"-B/s, "%s"%% quality "%s"-M4A.'
...
def transfer_metadata(source_file, target_file):
    ...
    if isinstance(additional_metadata, mutagen.flac.FLAC):
        ...
        if hasattr(additional_metadata, "pictures"):
            ...
            m4a_data["covr"] = [
                mutagen.mp4.MP4Cover(pic.data, mutagen.mp4.MP4Cover.FORMAT_JPEG)
                if "jpeg" in pic.mime
                else mutagen.mp4.MP4Cover(pic.data, mutagen.mp4.MP4Cover.FORMAT_PNG)
                for pic in additional_metadata.pictures
            ]

    elif isinstance(additional_metadata, mutagen.mp3.MP3):
        ...
        if "APIC:" in additional_metadata:
            ...
            m4a_data["covr"] = [
                mutagen.mp4.MP4Cover(
                    ...
                    if "jpeg" in additional_metadata["APIC:"].mime
                    else mutagen.mp4.MP4Cover.FORMAT_PNG,
                )
            ]

    elif isinstance(additional_metadata, mutagen.mp4.MP4):
...
def process_audio_file(audio_file):
    ...
    if audio_file.endswith(".flac"):
        m4a_file = (
            os.path.join(output_location, os.path.basename(audio_file[:-5])) + ".m4a"
        )
        wav_file = (
            os.path.join(tmp_location, os.path.basename(audio_file[:-5])) + ".wav"
        )
        convert_flac_to_wav(audio_file, wav_file)
        convert_wav_to_aac(wav_file, m4a_file)
        os.remove(wav_file)
        logger.debug(
            'Removed intermediate WAV file: "%s".' % os.path.basename(wav_file)
        )

    else:
        m4a_file = (
            os.path.join(output_location, os.path.basename(audio_file[:-4])) + ".m4a"
        )
